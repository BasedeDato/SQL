	...
	<body>
		...
		<?php
			...
			if($actividad != "Seleccione Una Actividad" && $maquina != "Seleccione Una Maquina" ){
				...
				$query = "SELECT nombre FROM cliente INNER JOIN actividad WHERE Nombre_Actividad ='".$actividad."' NOT IN (SELECT nombre FROM cliente INNER JOIN maquina WHERE Nombre_Maquina ='".$maquina."')";
			}else {
